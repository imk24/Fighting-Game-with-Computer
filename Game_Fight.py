...
#User inputs
    while hp_user >= 0 or hp_comp >= 0:
        ...
        while (user != 'slash') and (user != 'forwards') and (user != 'backwards') and (user!= 'dash'):
            user = input("Enter either: 'slash', 'forwards', 'backwards', or 'dash' ")
        
        
#User Movement
        if comp_turn == True or number == 0:
...
#User Conditions
            if user == 'slash':        
                ...
                if (d_comp - d_user) == 1 or (d_user - d_comp) == 1:
                    user_damage = random.choice([15, 8, 8, 8, 0])
                    hp_comp = hp_comp - user_damage 
                    if user_damage == 25:
...
#Computer Move
            if Tru == True:
                ...
                if (d_user - d_comp) == 1 or (d_user - d_comp) == -1:
                    ...
                    if comp_move == 'slash':
                        comp_damage = random.choice([15, 8, 8, 8, 0])
                        hp_user = hp_user - comp_damage
                        if comp_damage == 25:
